...
void oscEvent(OscMessage theOscMessage){
  ...
  println("mouseX:: "+theOscMessage.get(0).intValue());
  println("mouseY:: "+theOscMessage.get(1).intValue());
  netX = theOscMessage.get(0).intValue();
  netY = theOscMessage.get(1).intValue();
}
...
void newp(int p) {
  if(dodge) { 
    ...
    a[p] = new float[] { netX + r * cos(ang), netY + r *sin(ang) };
    if(frameCount %60== 0) {
